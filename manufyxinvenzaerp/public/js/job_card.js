const INSPECTION_OPERATIONS = ["Fitup Inspection", "Final Inspection"];

frappe.ui.form.on("Job Card", {
	refresh(frm) {
		if (frm.is_new() || !INSPECTION_OPERATIONS.includes(frm.doc.operation)) return;
		...
		if (!pending) {
			frm.add_custom_button(__("Add Inspection Call"), function () {
				...
				frappe.call({
					...
					args: { source_doctype: "Job Card", source_name: frm.doc.name },
					freeze: true,
